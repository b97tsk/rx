...
// CombineLatest8 creates an Observable that combines multiple Observables to
		...
		go func() {
			...
			for !exit {
				...
				case <-done:
					...
					return
				case n := <-chan1:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V1, 1)
				case n := <-chan2:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V2, 2)
				case n := <-chan3:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V3, 4)
				case n := <-chan4:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V4, 8)
				case n := <-chan5:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V5, 16)
				case n := <-chan6:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V6, 32)
				case n := <-chan7:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V7, 64)
				case n := <-chan8:
					exit = combineLatestSink8(n, sink, proj, &s, &s.V8, 128)
				}
			}
		}()

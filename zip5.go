...
// Zip5 creates an Observable that combines multiple Observables to create
		...
		go func() {
			...
			for !exit {
				...
				case <-done:
					...
					return
				case n := <-chan1:
					exit = zipSink5(n, sink, proj, &s, &s.Q1, 1)
				case n := <-chan2:
					exit = zipSink5(n, sink, proj, &s, &s.Q2, 2)
				case n := <-chan3:
					exit = zipSink5(n, sink, proj, &s, &s.Q3, 4)
				case n := <-chan4:
					exit = zipSink5(n, sink, proj, &s, &s.Q4, 8)
				case n := <-chan5:
					exit = zipSink5(n, sink, proj, &s, &s.Q5, 16)
				}
			}
		}()
...
func zipSink5[T1, T2, T3, T4, T5, R, X any](
	...
	case n.HasValue:
		...
		if s.RBits |= b; s.RBits == FullBits {
			...
			sink.Next(proj(
				zipPop5(s, &s.Q1, 1, &completed),
				zipPop5(s, &s.Q2, 2, &completed),
				zipPop5(s, &s.Q3, 4, &completed),
				zipPop5(s, &s.Q4, 8, &completed),
				zipPop5(s, &s.Q5, 16, &completed),
			))

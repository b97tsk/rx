	...
	return func(c Context, sink Observer[R]) {
		...
		c.Go(func() {
			...
			for cont {
				select {
				case n := <-chan1:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V1, 1)
				case n := <-chan2:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V2, 2)
				case n := <-chan3:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V3, 4)
				case n := <-chan4:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V4, 8)
				case n := <-chan5:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V5, 16)
				case n := <-chan6:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V6, 32)
				case n := <-chan7:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V7, 64)
				case n := <-chan8:
					cont = withLatestFromTry8(sink, n, mapping, &s, &s.V8, 128)
				}
			}
		})
	}

		...
		go func() {
			...
			for !exit {
				...
				case <-done:
					...
					return
				case n := <-chan1:
					exit = zipSink7(n, sink, proj, &s, &s.Q1, 1)
				case n := <-chan2:
					exit = zipSink7(n, sink, proj, &s, &s.Q2, 2)
				case n := <-chan3:
					exit = zipSink7(n, sink, proj, &s, &s.Q3, 4)
				case n := <-chan4:
					exit = zipSink7(n, sink, proj, &s, &s.Q4, 8)
				case n := <-chan5:
					exit = zipSink7(n, sink, proj, &s, &s.Q5, 16)
				case n := <-chan6:
					exit = zipSink7(n, sink, proj, &s, &s.Q6, 32)
				case n := <-chan7:
					exit = zipSink7(n, sink, proj, &s, &s.Q7, 64)
				}
			}
		}()

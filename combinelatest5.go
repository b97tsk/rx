...
// CombineLatest5 creates an Observable that combines multiple Observables to
		...
		go func() {
			...
			for !exit {
				...
				case <-done:
					...
					return
				case n := <-chan1:
					exit = combineLatestSink5(n, sink, proj, &s, &s.V1, 1)
				case n := <-chan2:
					exit = combineLatestSink5(n, sink, proj, &s, &s.V2, 2)
				case n := <-chan3:
					exit = combineLatestSink5(n, sink, proj, &s, &s.V3, 4)
				case n := <-chan4:
					exit = combineLatestSink5(n, sink, proj, &s, &s.V4, 8)
				case n := <-chan5:
					exit = combineLatestSink5(n, sink, proj, &s, &s.V5, 16)
				}
			}
		}()
